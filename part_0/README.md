...
activate browser
Note right of browser: The JavaScript code is executed<br/>to fetch the JSON data from the server
...
server-->>-browser: The CSS file main.css is sent
activate browser
Note right of browser: The JavaScript code is executed<br/>to fetch the JSON data from the server<br/>a
browser->>+server: GET https://studies.cs.helsinki.fi/exampleapp/data.json
deactivate browser
...
activate browser
Note right of browser: The JavaScript sends the note as a JSON object { "content": "New note", "date": "2023-05-16" }<br/>and renders the view dinymically appending the elements to the DOM wihtout sending a new GET request
deactivate browser
```